...
const utils = {
  ...
  dataURLtoBlob: (dataURL: string) => {
    try {
      ...
      return (
        "ERR_DOM Error on converting data URI to blob " +
        (error?.toString ? error.toString() : String(e))
      );
    }
  },
...
export const imgConvert = async (
  file: File,
  outputFormat = "image/jpeg",
  callback: (result: { file: File | null; fileUrl: string }) => void,
) => {
  const result = {
